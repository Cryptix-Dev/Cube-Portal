			...
			while (isRunning) {
				...
				while ((updateMillis = SystemClock.elapsedRealtime()) > nextGameTick && loops < MAX_FRAMESKIP) {
					...
					renderer.update(new GameTime(
												new TimeSpan((long)(updateMillis - oldUpdateMillis)),
												new TimeSpan((long)(updateMillis - startMillis)),
												loops != 0));
					loops++;
				...
				renderer.prepareDraw(new GameTime(new TimeSpan((long)(drawMillis - oldDrawMillis)),
												  new TimeSpan((long)(drawMillis - startMillis)),
												  loops != 0),
									interpolation);
				requestRender();
